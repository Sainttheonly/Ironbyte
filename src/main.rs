...
use crate::engine::enforce::{set_tunables, Tunables};
...
struct Policy {
    ...
    killset_cap: Option<u32>,
    budget_window_ns: Option<u64>,
    budget_global_limit: Option<u32>,
    budget_per_id_limit: Option<u32>,
}
...
fn main() -> Result<()> {
    ...
    if let Some(pol) = cfg.effective_policy() {
        ...
        set_tunables(Tunables {
            killset_cap,
            budget_window_ns: window_ns,
            budget_global_limit: global_lim,
            budget_per_id_limit: per_id_lim,
        });

        eprintln!(
            "[*] tunables wired: killset_cap={} window_ns={} global={} per_id={}",
            killset_cap, window_ns, global_lim, per_id_lim
        );
    } else {
        // No policy block: still set defaults explicitly so enforcement.rs never relies on implicit OnceLock state.
        set_tunables(Tunables {
            killset_cap: 256,
